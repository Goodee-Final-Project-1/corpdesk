...
public class AttendanceController {
    ...
    public String list(@RequestParam("username") String username, Model model)  throws Exception {
        ...
        // 3. 통계 데이터 조회 (지각, 조퇴, 결근 횟수, 근무 시간, 근무일수) (기준: 현재 년도&월)
        ResAttendanceDTO attCnts = attendanceService.getAttendanceCounts(username, currentYearStr, currentMonthStr);
        ResAttendanceDTO workSummary = attendanceService.getWorkSummary(username, currentYearStr, currentMonthStr);

        // 4. 상세 기록 목록 (기준: 현재 년도&월)
        List<ResAttendanceDTO> attDatilList = attendanceService.getAttendanceDetailList(username, currentYearStr, currentMonthStr);
